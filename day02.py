...
def sumDividend(s):
    lines = to_array(s)
    combinations = [itertools.combinations(line, 2) for line in lines]
    sum = 0
    for combs in combinations:
        pairs = np.array([[c[0], c[1]] for c in combs])
        mods = [np.array([p[0]%p[1], p[1]%p[0]]) for p in pairs]
        checkMods =  np.array([np.sum(modPair==0) for modPair in mods])
        idxMatch = np.where(checkMods>0)
        a,b = pairs[idxMatch[0]][0]
        if a%b == 0:
            sum += a//b
        else:
            sum += b//a

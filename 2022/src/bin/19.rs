...
fn max_geodes(
    (ore_ore_cost, clay_ore_cost, obs_ore_cost, obs_clay_cost, geo_ore_cost, geo_obs_cost): (
        u32,
        u32,
        u32,
        u32,
    ...
    let max_ore_cost = *[ore_ore_cost, clay_ore_cost, obs_ore_cost, geo_ore_cost]
        .iter()
        .max()
        .unwrap();
    ...
    while let Some((len, state)) = q.pop_front() {
        ...
        if state.obsidian_robots < geo_obs_cost
...
fn main(input: &str) -> (u32, u32) {
    ...
    let p1 = blueprints
        .par_iter()
        .enumerate()
        .map(|(i, &b)| max_geodes(b, 24) * (i as u32 + 1))
        .sum();
    let p2 = blueprints
        .par_iter()
        .take(3)
